...
class MainActivity : AppCompatActivity() {
    ...
    val database = Firebase.database("https://productdesignimplementat-9e640-default-rtdb.europe-west1.firebasedatabase.app")
    val dbRef = database.getReference("heartrate_record")

    // ATTENTION! Replace with the device ID from your device.
    private var deviceId = "50922921"

    private var currentHeartrate = 100
    private var targetHeartrate = 120
    private var monitorHeartrate = false
    private var recording = false
    private var currentSegment = "Sport"
    private var averageHeartrate: Double = -1.00
    private var minHeartrate = -1
    private var maxHeartrate = -1
    private val heartrateArray = arrayListOf<Int>()

    private var notificationDelayCounter = 0

    private val api: PolarBleApi by lazy {
        // Notice PolarBleApi.ALL_FEATURES are enabled
        PolarBleApiDefaultImpl.defaultImplementation(this, PolarBleApi.FEATURE_HR)
    }
    ...
    private lateinit var setTrainingHeartrateButton: Button
    private lateinit var monitorTrainingHeartrateButton: Button
    private lateinit var recordButton: Button
    private lateinit var segmentName: TextView
    private lateinit var averageHeartrateView: TextView
    private lateinit var maxHeartrateView: TextView
    private lateinit var minHeartrateView: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setTrainingHeartrateButton = findViewById(R.id.setTrainingHeartrateButton)
        monitorTrainingHeartrateButton = findViewById(R.id.monitorTrainingHeartrateButton)
        recordButton = findViewById(R.id.recordButton)
        segmentName = findViewById(R.id.segmentName)
        averageHeartrateView = findViewById(R.id.averageHeartrate)
        maxHeartrateView = findViewById(R.id.maxHeartrate)
        minHeartrateView = findViewById(R.id.minHeartrate)
        ...
        api.setApiCallback(object : PolarBleApiCallback() {
            ...
            override fun hrNotificationReceived(identifier: String, data: PolarHrData) {
                ...
                heartrateTextView.setText(currentHeartrate.toString())

                if (monitorHeartrate and (abs(targetHeartrate-currentHeartrate)>10) ){
                    if(notificationDelayCounter > 5){
                        notificationDelayCounter = 0
                        val notification: Uri =
                            RingtoneManager.getDefaultUri(RingtoneManager.TYPE_NOTIFICATION)
                        val r = RingtoneManager.getRingtone(applicationContext, notification)
                        r.play()
                    }else{
                        notificationDelayCounter += 1
                    }
                }
                ...
                if (recording){
                    if ( currentHeartrate > maxHeartrate ) {
                        ...
                        maxHeartrateView.setText(maxHeartrate.toString())
                    }
                    if ( currentHeartrate < minHeartrate ) {
                        minHeartrate = currentHeartrate
                        minHeartrateView.setText(minHeartrate.toString())
                    }
                    heartrateArray.add(currentHeartrate)
                    averageHeartrate = round(heartrateArray.average()*100)/100
                    averageHeartrateView.setText(averageHeartrate.toString())
                }
            }
        ...
        recordButton.setOnClickListener {
            if(!recording){
                recording = true
                toggleButtonDown(recordButton, R.string.stop_recording)

                averageHeartrate = currentHeartrate.toDouble()
                heartrateArray.add(currentHeartrate)
                minHeartrate = currentHeartrate
                maxHeartrate = currentHeartrate
                averageHeartrateView.setText(averageHeartrate.toString())
                ...
                dialogBuilder.setTitle("set current Activity")
                ...
                dialogBuilder.setCancelable(true)
                    ...
                    .setPositiveButton("OK", DialogInterface.OnClickListener {dialog,id ->
                        currentSegment = input.text.toString()
                        segmentName.setText(currentSegment)
                    })
                    .show()
            }else{
                recording = false
                toggleButtonUp(recordButton, R.string.start_recording)
                val tmp = System.currentTimeMillis().toString()
                dbRef.child(tmp).setValue( uploadModel(currentSegment, heartrateArray.toString()) )
                //segmentName.setText(R.string.current_activity)
            }
        }

        monitorTrainingHeartrateButton.setOnClickListener {
            if(!monitorHeartrate){
                monitorHeartrate = true
                toggleButtonDown(monitorTrainingHeartrateButton, R.string.monitor_training_hr)
            }else{
                monitorHeartrate = false
                toggleButtonUp(monitorTrainingHeartrateButton, R.string.monitor_training_hr)
            }
        }

        setTrainingHeartrateButton.setOnClickListener {
            val dialogBuilder = AlertDialog.Builder(this)
            dialogBuilder.setTitle("set training Heartrate")
            val input = EditText(this)
            ...
            dialogBuilder.setCancelable(true)
                ...
                .setPositiveButton("OK", DialogInterface.OnClickListener {dialog,id ->
                    targetHeartrate = input.text.toString().toInt()
                    targetHeartrateTextView.setText(targetHeartrate.toString())
                })
                .show()
